...
const DialogConnections: React.FC<DialogConnectionsProps> = ({
  show,
  connections,
  connectionCurrId,
  closeDialog,
  ...
  return (
    <Box>
      <Dialog onClose={closeDialog} open={show} fullScreen TransitionComponent={Transition} >
        ...
        <DialogContent>
            ...
            {connections.map((connection) => {
              return (
                <Grid item xs={6} sm={4} md={3} key={connection.id}>
                  <SingleConnection connection={connection} highlighted={connectionCurrId === connection.id} handleClick={handleSetConnection} />
                </Grid>
              );
            })}
          </Grid>
          ...
          <Box marginY="20px">
            ...
            <Typography sx={{ ...styles.smallTextThin }}>
              &#42;Цены за подключения указаны в Евро. Оплата по курсу ЦБ +2%.
              Данные цены носят информативный характер. Пожалуйста, уточняйте
              акутальную стоимость у продавцов.
            </Typography>
